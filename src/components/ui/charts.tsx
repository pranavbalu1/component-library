...
export function StackedBarGraph({
  ...
  const [selectedTimeframe, setSelectedTimeframe] = React.useState(defaultTimeframe)
  const [isDropdownOpen, setIsDropdownOpen] = React.useState(false)

  const [selectedIndex, setSelectedIndex] = React.useState<number>(1)
  const [hoveredColIndex, setHoveredColIndex] = React.useState<number | null>(null)
  ...
  const maxStackValue = React.useMemo(() => {
    ...
    const highestTotal = Math.max(
      ...activeData.map((col) => col.segments.reduce((acc, s) => acc + s.value, 0))
    )
  ...
  const getRadiusStyle = (position: "top" | "bottom" | "middle" | "single") => {
    const r = typeof cornerRadius === "number" ? `${cornerRadius}px` : cornerRadius
        ...
        <div className="flex items-center gap-2">
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <ChevronDown className={cn("size-3.5 transition-transform duration-200", isDropdownOpen && "rotate-180")} />
            </button>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected ? "bg-zinc-800 text-white font-semibold" : "text-zinc-400 hover:text-white hover:bg-zinc-800/50"
                      )}
                    >
                      <span>{option}</span>
                      {isSelected && <Check className="size-3.5 text-[#e6ff4b]" />}
                    </button>
                  )
                })}
              </div>
      ...
      <div className="relative flex items-stretch gap-4 h-60 pt-6 my-2">
        ...
        <div className="flex-1 flex items-end justify-between gap-3 h-full px-2">
          {activeData.map((col, colIdx) => {
            ...
            const totalHeightPercent = Math.min((totalVal / maxStackValue) * 100, 100)

            const topSegment = col.segments[col.segments.length - 1]
            const topColorStyle = topSegment ? parseColorStyle(topSegment.color) : parseColorStyle("#e6ff4b")

            const showHighlight = isHoveredCol || (isSelected && hoveredColIndex === null)
              ...
              >
                ...
                >
                  <div className="w-full h-full flex flex-col-reverse gap-[2px]">
                    {col.segments.map((seg, segIdx) => {
                      const segmentPercent = totalVal > 0 ? (seg.value / totalVal) * 100 : 0
                      const isTop = segIdx === col.segments.length - 1
                      const isBottom = segIdx === 0
                      const pos =
                        isTop && isBottom ? "single" : isTop ? "top" : isBottom ? "bottom" : "middle"
                ...
                <span
                  className={cn(
                    ...
                    isSelected || isHoveredCol ? "text-white font-bold" : "text-zinc-500"
                  )}
                >
                  {col.label}
                </span>
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center justify-center my-2 h-44">
        <svg viewBox="0 0 200 115" className="w-full max-w-[220px] overflow-visible">
          <path
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <span className={cn("font-medium transition-colors", isSelected ? "text-white font-bold" : "text-zinc-300")}>
                  {cat.label}
                </span>
              </div>
              <span className={cn("font-bold transition-colors", isSelected ? "text-white" : "text-zinc-400")}>
                {cat.percentage}%
              </span>
            </div>
          )
